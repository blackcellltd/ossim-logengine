...
class JSON(tornado.web.RequestHandler):
    ...
    def post(self):
                ...
                for filt in ors["$or"]:
                    ...
                    if "timestamp" in filt:
                        if "$gte" in filt["timestamp"]:
                            ...
                            query += str(names["timestamp"]) + " greater than " + filt["timestamp"][
                                "$gte"].isoformat() + "<p>"
                        if "$lte" in filt["timestamp"]:
                            filt["timestamp"]["$lte"] = parser.parse(filt["timestamp"]["$lte"])
                            query += str(names["timestamp"]) + " less than " + filt["timestamp"][
                                "$lte"].isoformat() + "<p>"
        ...
        def cb():
            cursor = db.log.find(filter).sort("timestamp", -1)
...
class ActivePlugins(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        for key in self.ossim_keys:
            try:
                plugin_max_lines = max_line_counter("/usr/share/python/ossim-logserver/activePlugins.list")
                sids_max_lines = max_line_counter("/usr/share/python/ossim-logserver/activeSids.list")
                server_logger.info("Acquired number of lines.")
                try:
                    plugins_list = open("/usr/share/python/ossim-logserver/activePlugins.list", 'r')
                    sids_list = open("/usr/share/python/ossim-logserver/activeSids.list", 'r')
                    server_logger.info("Successfully opened list files.")

                    activePlugins = []
                    while i < plugin_max_lines:
                        activePlugins.append(int(plugins_list.readline()))
                        i += 1

                    i = 0
                    activePairs = []
                    while i < sids_max_lines:
                        reform_sids.append(sids_list.readline())
                        a_first = reform_sids[i].find('(') + 1
                        b_first = reform_sids[i].find(',')
                        a_second = reform_sids[i].find(' ') + 1
                        b_second = reform_sids[i].rfind(')')
                        activePairs.append((int(reform_sids[i][a_first:b_first]), int(reform_sids[i][a_second:b_second])))
                        i += 1
                    ...
                    server_logger.error("Failed to open list files.")

            except IOError:
                server_logger.error("Failed to acquire maximum number of lines in lists")
